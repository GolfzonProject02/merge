...
import worktalk.com.user.controller.HomeController;
 ...
 * Creating random code for email validation.
 * Send Mail to join member email.
 *
 */
...
public class MailSenderServiceimpl implements MailSenderService {
	...
	/*
	 * call for dao.checkDuplicatedMail in oreder to find any duplicate mail in MEMBER table
	 * returns 1 if table has duplicated email
	 */
	@Override
	public int checkDuplicatedEmail(User user) {
		...
		return dao.checkDuplicateMail(user);
	}
	...
	/*
	 * create an email form containing the validation code(6 digit random num) and sends it
	 * if the email address is duplicated => returns "Duplicated Email"
	...
	public String joinMail(String email) {
		...
		int flag = checkDuplicatedEmail(user);
		...
		if (flag == 0) {
			...
			String content = 
					"홈페이지를 방문해주셔서 감사합니다." + 	//Should be written in html
							"<br><br>" + 
							"인증 번호는 " + String.valueOf(code) + "입니다." + 
							"<br>" + 
